...
const ITEMS: Qa[] = [
  {
    ...
    a: "Подвоха нет: на старте платформа полностью бесплатна, без карты и скрытых платных ограничений. Мы сначала растим живую аудиторию, монетизацию придумаем потом — и скажем об этом заранее. Единственное ограничение — честный дневной лимит ИИ-генераций, потому что ИИ стоит денег. Лимит виден в интерфейсе, а не спрятан в справке.",
  },
  {
    icon: Scale,
    q: "Это законно — следить за конкурентами?",
    a: "Мы собираем только открытые данные — то, что видит любой подписчик, — и только через официальные подключения соцсетей. Ничего закрытого, никаких персональных данных, никакого сбора в обход правил. Это то же, что ты можешь посмотреть руками — просто платформа делает это за тебя и подводит итог.",
  },
  {
    icon: KeyRound,
