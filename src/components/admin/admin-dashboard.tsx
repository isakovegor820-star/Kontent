...
const SECTION_COPY: Record<AdminSection, { eyebrow: string; title: string; description: string }> = {
  overview: { eyebrow: "Пульс Авроры", title: "Обзор", description: "Состояние публикаций, подключений и активности клиентов — без демонстрационных данных." },
  ...
  projects: { eyebrow: "Тенанты", title: "Проекты", description: "Владелец, команда, каналы, публикации, автопилот и бот каждого рабочего пространства." },
  "bot-control": { eyebrow: "Telegram", title: "Управление ботом", description: "Состояние бота, пользователи, проекты и журнал — с обратимыми bot-only действиями." },
  system: { eyebrow: "Платформа", title: "Состояние системы", description: "Только подтверждённое состояние зависимостей: настроено ≠ работает." },
  "aurora-analytics": { eyebrow: "Продукт и качество", title: "Аналитика Авроры", description: "Использование, техническое здоровье и доменный результат всех разделов из APP_ROUTES." },
...
function MetricCard({
  ...
  if (href) {
    return (
      <a href={href} className="card-plain block rounded-md p-3.5 transition-[border-color,transform] duration-150 hover:-translate-y-0.5 hover:border-brand/35 motion-reduce:transform-none sm:p-5">
        {body}
      </a>
    );
  }
...
export function AdminDashboard() {
  ...
  const showPeriodControls = activeSection !== "system" && activeSection !== "aurora-analytics" && activeSection !== "audit";
        ...
        <header className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
          ...
          {showPeriodControls ? (
            <div className="flex flex-wrap items-center gap-2">
              <fieldset className="inline-flex rounded-sm border border-line bg-surface p-1" aria-label="Период сравнения">
            ...
            {data && pulse ? (
              <>
                <div className={cn(
                  ...
                  <p className="type-caption shrink-0 text-text-3">Проверено {fmtAgo(data.checkedAt)}</p>
                </div>
                ...
                <div className="mt-4 grid grid-cols-2 gap-3 sm:gap-4 xl:grid-cols-4">
                  ...
                  <MetricCard label="Заходили за 30 дней" value={data.summary.activeUsers} helper="Сессия ещё не истекла" icon={Activity} tone="brand" href={adminUsersHref("/admin", { status: "active" })} />
                  ...
                  <MetricCard label="Опубликовано сегодня" value={data.summary.publishedToday} helper={`${fmtNum(data.summary.scheduled)} запланировано`} icon={Send} tone="brand" href={adminPublicationsHref("/admin", { pstatus: "scheduled" })} />
                  ...
                  <MetricCard label="AI сегодня" value={data.summary.aiToday} helper={`${fmtNum(data.summary.aiPeriod)} за период`} icon={Sparkles} href="/admin?system=aurora_ai#system" />
                  <MetricCard label="Переподключить" value={data.summary.authAttention} helper="Каналы с ошибкой доступа" icon={Radio} tone={data.summary.authAttention > 0 ? "danger" : "neutral"} href={adminUsersHref("/admin", { status: "attention" })} />
                </div>

                <div className="mt-4">
                  <AdminInbox data={data} onChanged={requestRefresh} />
                </div>
                      ...
                      {data.providers.length === 0 ? (
                        ...
                        <li key={provider.network} className="flex items-center justify-between gap-4 rounded-sm border border-line px-3 py-2.5">
                          <div className="min-w-0">
                            ...
                            <p className="type-caption text-text-3">{fmtNum(provider.active)} из {fmtNum(provider.total)} работают</p>
                          </div>
