...
export const LEGAL_VISUAL_DIMENSIONS: Record<LegalVisualFormat, LegalVisualDimensions> = {
  "1:1": {
    width: 1_080,
    height: 1_080,
...
export type LegalVisualWarningCode =
  | "title_overflow"
  | "theses_overflow"
  | "emphasis_overflow"
  | "cta_overflow"
  ...
  | "sequence_hook_missing"
  | "sequence_cta_missing"
  | "asset_unresolved"
  | "asset_hash_mismatch"
  | "asset_invalid";
...
export type LegalVisualAssetResolver = (
  reference: LegalVisualAssetReference,
) => Promise<LegalVisualResolvedAsset | null>;
...
type SvgContext = {
  ...
  card: LegalVisualCard;
  cardIndex: number;
  dimensions: LegalVisualDimensions;
...
const FONT_FAMILIES: Record<LegalVisualFontKey, string> = {
...
const TEMPLATE_BODY_FACTORS: Record<LegalVisualTemplateKey, number> = {
  what_changed: 0.84,
  three_actions: 0.83,
  deadlines: 0.78,
...
function layoutLimits(format: LegalVisualFormat, template: LegalVisualTemplateKey): LayoutLimits {
...
export function escapeLegalVisualXml(value: string) {
  return value
    .replaceAll("&", "&amp;")
    .replaceAll("<", "&lt;")
    .replaceAll(">", "&gt;")
...
function allWrappedLines(value: string, maxWidth: number, fontSize: number) {
  ...
  for (const paragraph of value.replace(/\r\n?/gu, "\n").split("\n")) {
    ...
    const tokens = paragraph.trim().split(/\s+/u).flatMap((token) =>
      textUnits(token) > maxUnits ? splitLongToken(token, maxUnits) : [token]
    );
    let line = "";
...
function wrapText(value: string, maxWidth: number, fontSize: number, maxLines: number): WrappedText {
  ...
  if (allLines.length > maxLines && visible.length > 0) {
    ...
    const maxUnits = Math.max(2, maxWidth / fontSize);
    while (last && textUnits(`${last}…`) > maxUnits) last = Array.from(last).slice(0, -1).join("");
...
function inspectCard(config: LegalVisualConfig, card: LegalVisualCard) {
  const limits = layoutLimits(config.format, card.template);
  const warnings: LegalVisualLayoutWarning[] = [];
  ...
  if (title.overflow) {
    warnings.push(warning(
      ...
      card.id,
      "title",
      "Заголовок не помещается в шаблон",
      title.totalLines,
      limits.titleMaxLines,
    ));
  }
  ...
  for (let index = 0; index < card.theses.length; index += 1) {
    const wrapped = wrapText(
      ...
      limits.bodyWidth,
      limits.bodyFontSize,
      limits.thesisMaxLines,
    );
    totalBodyLines += wrapped.totalLines;
    if (wrapped.overflow) thesisOverflow = true;
  }
  if (thesisOverflow || totalBodyLines > limits.bodyMaxLines) {
    warnings.push(warning(
      ...
      card.id,
      "theses",
      "Тезисы выходят за доступную область",
      totalBodyLines,
      limits.bodyMaxLines,
    ));
  }
  ...
  if (card.emphasis) {
    const emphasis = wrapText(
      card.emphasis,
      limits.bodyWidth,
      limits.emphasisFontSize,
      limits.emphasisMaxLines,
    );
    if (emphasis.overflow) {
      warnings.push(warning(
        ...
        card.id,
        "emphasis",
        "Акцент не помещается в выделенную область",
        emphasis.totalLines,
        limits.emphasisMaxLines,
      ));
    }
  ...
  if (card.cta) {
    ...
    if (cta.overflow) {
      warnings.push(warning(
        ...
        card.id,
        "cta.label",
        "Призыв к действию слишком длинный",
        cta.totalLines,
        limits.ctaMaxLines,
      ));
    }
  ...
  if (
    card.theses.length < template.recommendedTheses.min ||
    ...
    warnings.push(warning(
      ...
      card.id,
      "theses",
      `Для шаблона «${template.name}» лучше ${template.recommendedTheses.min}–${template.recommendedTheses.max} тезиса`,
      card.theses.length,
      template.recommendedTheses.max,
    ));
  }
  ...
  if (warnings.some((item) => item.severity === "error")) {
    warnings.push(warning(
      ...
      card.id,
      "layout",
      "Часть текста может выйти за безопасную область",
    ));
  }
  return warnings;
}
...
export function inspectLegalVisualConfig(value: LegalVisualConfig): LegalVisualLayoutWarning[] {
  ...
  if (contrastRatio(colors.text, colors.background) < 4.5) {
    warnings.push(warning(
      ...
      null,
      "brand.colors.text",
      "Контраст основного текста и фона ниже 4,5:1",
    ));
  }
  if (contrastRatio(colors.text, colors.surface) < 4.5) {
    warnings.push(warning(
      ...
      "Контраст текста на дополнительном фоне ниже 4,5:1",
    ));
  }
  ...
  if (signatureLines > 2) {
    warnings.push(warning(
      ...
      null,
      "brand.signature",
      "Подпись бренда не помещается в нижнюю область",
      signatureLines,
      2,
    ));
  }
  if (config.cards[0]?.role !== "hook") {
    warnings.push(warning(
      "sequence_hook_missing",
      "warning",
      config.cards[0]?.id ?? null,
      "cards.0.role",
      "Первая карточка обычно должна быть зацепкой",
    ));
  }
  if (config.cards.at(-1)?.role !== "cta") {
    warnings.push(warning(
      "sequence_cta_missing",
      "warning",
      config.cards.at(-1)?.id ?? null,
      `cards.${Math.max(0, config.cards.length - 1)}.role`,
      "Последняя карточка обычно завершает сценарий призывом",
    ));
  }
...
function svgText(
  ...
  const tspans = lines.map((line, index) =>
    `<tspan x="${options.x}" dy="${index === 0 ? 0 : lineHeight}">${escapeLegalVisualXml(line)}</tspan>`
  ).join("");
...
function imageFrame(
  reference: LegalVisualAssetReference | null,
  dataUrls: ReadonlyMap<string, string>,
  x: number,
  y: number,
  width: number,
...
function commonHeader(context: SvgContext, options: { titleAfter?: number } = {}) {
...
function visibleThesisLines(context: SvgContext, thesis: string, width: number, maxLines?: number) {
  return wrapText(
    thesis,
    width,
    context.limits.bodyFontSize,
    maxLines ?? context.limits.thesisMaxLines,
  ).lines;
}

function renderChangeSplit(context: SvgContext) {
...
function renderNumberedSteps(context: SvgContext) {
  const { dimensions, colors, fontFamily, card } = context;
...
function renderTimeline(context: SvgContext) {
  const { dimensions, colors, fontFamily, card } = context;
...
function renderRiskNotice(context: SvgContext) {
  const { dimensions, colors, fontFamily, card } = context;
...
function renderJudicialQuote(context: SvgContext) {
  const { dimensions, colors, fontFamily, card } = context;
...
function renderMythFact(context: SvgContext) {
  const { dimensions, colors, fontFamily, card } = context;
...
function renderChecklist(context: SvgContext) {
  const { dimensions, colors, fontFamily, card } = context;
...
function renderQuestionAnswer(context: SvgContext) {
  const { dimensions, colors, fontFamily, card } = context;
  const top = dimensions.height === 1_080 ? 420 : dimensions.height === 1_350 ? 500 : 690;
...
function renderKeyNumber(context: SvgContext) {
  const { dimensions, colors, fontFamily, card } = context;
...
function renderAnnouncement(context: SvgContext) {
  const { dimensions, colors, fontFamily, card } = context;
  const top = dimensions.height === 1_080 ? 420 : dimensions.height === 1_350 ? 500 : 690;
...
function renderCaseStudy(context: SvgContext) {
  const { dimensions, colors, fontFamily, card } = context;
  const top = dimensions.height === 1_080 ? 420 : dimensions.height === 1_350 ? 500 : 690;
  const bottom = dimensions.height - dimensions.safeArea.bottom - 120;
...
function renderTemplateBody(context: SvgContext) {
 ...
 */
export function buildLegalVisualCardSvg(
  value: LegalVisualConfig,
  cardId: string,
  dataUrls: ReadonlyMap<string, string> = new Map(),
) {
  const config = validateLegalVisualConfig(value);
  const cardIndex = config.cards.findIndex((candidate) => candidate.id === cardId);
  if (cardIndex < 0) throw new Error(`Unknown legal visual card: ${cardId}`);
...
async function resolveAssets(
  config: LegalVisualConfig,
  resolver: LegalVisualAssetResolver | undefined,
) {
  ...
  if (!resolver) return { dataUrls, warnings };

  for (const reference of references) {
    let resolved: LegalVisualResolvedAsset | null = null;
    try {
      ...
      warnings.push(warning(
        ...
        `Медиафайл «${reference.alt || reference.assetId}» недоступен`,
      ));
      continue;
    }
    if (!resolved) {
    ...
    if (createHash("sha256").update(input).digest("hex") !== reference.sha256) {
      warnings.push(warning(
        ...
        null,
        `asset.${reference.assetId}`,
        `Контрольная сумма медиафайла «${reference.alt || reference.assetId}» не совпала`,
      ));
      continue;
    }
    try {
      ...
      warnings.push(warning(
        ...
        null,
        `asset.${reference.assetId}`,
        `Медиафайл «${reference.alt || reference.assetId}» не удалось декодировать`,
      ));
    }
  }
...
export async function renderLegalVisualCarousel(
  value: LegalVisualConfig,
  options: RenderLegalVisualOptions = {},
): Promise<RenderedLegalVisualCarousel> {
  const config = validateLegalVisualConfig(value);
  ...
  const warnings = [...layoutWarnings, ...assets.warnings].sort((left, right) =>
  ...
  const dimensions = LEGAL_VISUAL_DIMENSIONS[config.format];
  const cards: RenderedLegalVisualCard[] = [];
