...
describe("article approval admission", () => {
  ...
  it("rejects a second approval of the same version without counting or rewriting it", async () => {
    ...
    await expect(approveSiteArticle({ query } as never, { site: { id: 5, project_id: 3, verification_state: "unverified" }, article: { ...article, project_id: 3 }, userId: 9 } as never))
      .rejects.toMatchObject({ code: "article_not_approvable", status: 409 });
    expect(query.mock.calls.some(([sql]) => sql.includes("update site_articles"))).toBe(false);
