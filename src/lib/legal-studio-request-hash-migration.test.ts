...
describe("legal studio request-hash migration", () => {
  it("is additive, transactional, legacy-safe, and mirrored by bootstrap schema", async () => {
    const sql = await readFile(
      resolve(process.cwd(), "db/migrations/20260822_legal_studio_request_hashes.sql"),
      "utf8",
    );
    ...
    expect(sql).toContain("alter table legal_visual_designs");
    expect(sql).toContain("alter table legal_video_scripts");
    expect(sql.match(/add column if not exists request_hash char\(64\)/gu)).toHaveLength(2);
    expect(sql).not.toMatch(/request_hash char\(64\)\s+not null/iu);
    expect(sql.match(/check \(request_hash is null or request_hash ~ '\^\[0-9a-f\]\{64\}\$'\)/gu))
      .toHaveLength(2);
