...
export async function reconcileLegalVisualRenderOutbox({
  pool,
  enqueue,
  operationId = null,
  limit = 100,
  ...
  await pool.query(
    `with stale as (
       update legal_visual_render_operations operation
          set status = 'retryable_failed', error_code = 'render_lease_expired',
              error_message = 'Рендер будет повторён автоматически.', updated_at = now()
        where operation.status = 'rendering'
          and operation.updated_at <= now() - interval '15 minutes'
          and ($1::bigint is null or operation.id = $1)
          and exists (
            select 1 from legal_visual_render_outbox outbox
             where outbox.operation_id = operation.id
               and outbox.project_id = operation.project_id
               and outbox.status not in ('failed','cancelled','completed')
          )
        returning operation.id, operation.project_id
     )
     update legal_visual_render_outbox outbox
  ...
  while (scanned < bounded) {
    ...
    try {
      ...
      row = (await client.query(
        `select outbox.id, outbox.operation_id, outbox.project_id,
                outbox.status as outbox_status, outbox.attempts, operation.config_hash
           from legal_visual_render_outbox outbox
           join legal_visual_render_operations operation
    ...
    try {
      ...
      const code = String(error?.code || "queue_unavailable")
        .replace(/[^a-z0-9_:-]/giu, "_").slice(0, 100);
      const updated = await pool.query(
        `update legal_visual_render_outbox
      ...
      if (updated.rowCount === 1) {
        await pool.query(
          ...
          [
            ...
            code,
            terminal
              ? "Не удалось передать рендер в очередь после повторных попыток."
              : "Рендер ожидает восстановления очереди.",
          ],
        );
        failed += 1;
      }
