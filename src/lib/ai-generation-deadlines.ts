...
export function generationDeadlines(
  ...
  if (mode === "fast") {
    return {
      firstTokenMs: configuredMs("AI_FAST_FIRST_TOKEN_MS", 10_000, env),
      attemptOverallMs: configuredMs("AI_FAST_OVERALL_MS", 60_000, env),
      pipelineOverallMs: configuredMs("AI_FAST_PIPELINE_MS", 60_000, env),
    };
  }
  if (mode === "maximum") {
    return {
