...
export function buildQualityPrompt(rawQuality, options = {}) {
  ...
  const desiredLengthInstruction = q.desiredMinChars != null && q.desiredMaxChars != null
    ? `— желаемый объём: ${q.desiredMinChars}–${q.desiredMaxChars} знаков с пробелами в готовом тексте, без служебных ссылок; цель — середина диапазона, с законченной мыслью и без повторов для добивки длины;`
...
export function buildRewritePrompt(draft, result) {
  ...
  const focusedInstructions = [
    ...(codes.has("hook")
      ? ["Исправь первую строку: короткий законченный хук в пределах указанного лимита. Отдели его пустой строкой. Содержательные подробности перенеси в основной текст, не удаляй их."]
      : []),
    ...(codes.has("too_short")
      ? [
          "Раскрой 2–3 конкретных аспекта темы, опираясь только на факты из источников системного сообщения.",
          ...(result?.desiredLength
            ? [`Сейчас ${result.desiredLength.actualChars} знаков; готовый текст должен занимать ${result.desiredLength.minChars}–${result.desiredLength.maxChars} знаков без служебных ссылок. Цель — около ${Math.round((result.desiredLength.minChars + result.desiredLength.maxChars) / 2)} знаков.`]
            : []),
          "Не добирай объём повторениями, общими вопросами или фразами о том, как написан пост.",
        ]
      : []),
    ...(codes.has("too_long")
  ...
  return [
    ...
    "Если после удаления замечаний текст короче требуемого объёма, раскрой другие содержательные аспекты темы по фактам из системного списка. Не заменяй содержание набором вопросов читателю.",
