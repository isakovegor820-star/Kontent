...
function errorLabel(error: unknown) {
  ...
  return mediaStorageErrorLabel(code) ?? labels[code] ?? "Не удалось выполнить действие. Проверьте соединение и попробуйте ещё раз.";
}
    ...
    >
      <div className="absolute inset-[7%] flex flex-col">
        ...
        {card.image && (
          ...
          <img src={projectNativeUrl(`/api/media/assets/${card.image.assetId}`)} alt={card.image.alt} className="mt-[5%] min-h-0 flex-1 rounded-sm object-cover" />
        )}
        ...
        <div className={cn("flex min-w-0 items-center gap-2 pt-[4%]", card.cta ? "" : "mt-auto")}>
          {config.brand.logo && (
            // eslint-disable-next-line @next/next/no-img-element -- authenticated project media cannot use the image optimizer
            <img
              src={projectNativeUrl(`/api/media/assets/${config.brand.logo.assetId}`)}
              alt={config.brand.logo.alt}
              className="h-[clamp(24px,6vw,44px)] w-[clamp(24px,6vw,44px)] shrink-0 rounded-xs object-contain outline outline-1 outline-[var(--image-outline)]"
            />
          )}
            ...
            <div className="mt-4 flex snap-x gap-3 overflow-x-auto pb-3">
              {render.cards.map((card) => (
                <button type="button" key={card.id} className="w-36 shrink-0 snap-start rounded-sm border border-line bg-surface-2 p-2 text-left focus-visible:ring-4 focus-visible:ring-brand/15" onClick={() => setActiveCardId(card.id)}>
                  {/* eslint-disable-next-line @next/next/no-img-element -- authenticated project media cannot use the image optimizer */}
                  <img src={projectNativeUrl(card.url)} alt={`Готовая карточка ${card.order}`} className="aspect-[4/5] w-full rounded-xs object-cover" />
      ...
      <div className="border-t border-line p-4 sm:p-5">
        <div className="grid min-w-0 gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(280px,0.56fr)]">
          ...
          <section aria-labelledby="brand-logo-heading" className="min-w-0 rounded-sm bg-surface-2 p-4">
            ...
            <div className="mt-4 flex min-h-24 items-center justify-center rounded-sm bg-surface p-3 outline outline-1 outline-[var(--image-outline)]">
              {brand.logo ? (
                // eslint-disable-next-line @next/next/no-img-element -- authenticated project media cannot use the image optimizer
                <img src={projectNativeUrl(`/api/media/assets/${brand.logo.assetId}`)} alt={brand.logo.alt} className="max-h-24 max-w-full object-contain" />
              ) : (
