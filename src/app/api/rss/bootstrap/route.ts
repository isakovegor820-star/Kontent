...
export async function POST(req: NextRequest) {
  ...
  try {
      ...
      const inserted = await pool.query<{ id: string }>(
        `insert into rss_feeds (
         user_id, channel_id, url, title, is_active, ai_summarize,
           publish_existing, source_kind, max_per_day, last_fetched_at
         )
         values ($1, $2, $3, $4, true, true, false, 'legal_opportunity', 3, null)
         ...
         returning id`,
        [user.id, wantedChannelId, source.url, source.title],
      );
    ...
    return NextResponse.json({ ok: true, connected, refreshQueued });
  } catch (error) {
