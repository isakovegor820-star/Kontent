...
export async function POST(req: NextRequest) {
  ...
  // 2. Сохраняем/обновляем канал. Группа принадлежит одному аккаунту (частичный unique
  //    в схеме на активных); гонку между select и insert ловим кодом 23505, как у TG.
  try {
    return await withSelectedProjectPermission(pool, user.id, "project.manage", async (client, membership) => {
      if (membership.projectId !== projectId) throw new ProjectAccessError("invalid_project_selector");
      const existing = await client.query<{ id: number; user_id: number }>(
        `select id, user_id from channels where project_id = $1 and vk_group_id = $2 for update`,
        [projectId, group.groupId],
      );
      // The worker decrypts with the stored channel owner, which can differ from
      // the current authorized project manager reconnecting this credential.
      const encrypted = encryptToken(token, { userId: existing.rows[0]?.user_id ?? user.id, provider: "vk" });
