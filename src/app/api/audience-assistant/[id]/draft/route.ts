...
import { discardAudienceReply } from "@/lib/audience-assistant";
...
export async function DELETE(request: NextRequest, { params }: Context) {
  ...
  const rate = await checkRateLimit(`audience-assistant:discard:user:${user.id}`, 360, 3_600, { failureMode: "closed" });
  ...
  try {
    const inquiry = await discardAudienceReply({
      actorUserId: user.id,
      inquiryId,
