...
async function readDecisionBody(req: Request) {
  if (req.headers.get("content-type")?.split(";", 1)[0].trim().toLowerCase() !== "application/json") {
    return null;
  }
  try {
    const bytes = await readRequestBodyLimited(req.body, MAX_DECISION_BODY_BYTES);
    const value = JSON.parse(new TextDecoder("utf-8", { fatal: true }).decode(bytes)) as unknown;
    if (!value || typeof value !== "object" || Array.isArray(value)) return null;
    const body = value as Record<string, unknown>;
    return Object.keys(body).every((key) => ["expectedVersion", "decision", "note"].includes(key))
      ? body
      : null;
  } catch (error) {
...
function errorResponse(error: unknown, requestId: string) {
  ...
  console.error("[publication-review-decision] failed", {
    requestId,
...
export async function POST(req: NextRequest, ctx: RouteContext) {
  ...
  const rate = await checkRateLimit(`publication-review:user:${user.id}`, 120, 3_600, {
    failureMode: "closed",
  });
  ...
  try {
    const result = await decidePublicationReview({
      pool: getPool(),
      actorUserId: user.id,
      reviewTaskId: (await ctx.params).id,
