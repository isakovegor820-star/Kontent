...
import { startAudienceQuestionDraft } from "@/lib/audience-questions";
...
export async function POST(request: NextRequest, { params }: Context) {
  ...
  const rate = await checkRateLimit(`audience-question:start:user:${user.id}`, 60, 3_600, { failureMode: "closed" });
  if (!rate.allowed) return rateLimitResponse(rate);
  const parsed = await audienceQuestionBody(request, ["expectedVersion"]);
  if (!parsed.ok) return audienceQuestionBodyFailure(parsed.error, requestId);
  try {
    const question = await startAudienceQuestionDraft({
      actorUserId: user.id,
      questionId,
