...
describe("GET /api/site-analysis/:id", () => {
  ...
  it("scopes the result to its owner and returns the persisted request ID", async () => {
    mocks.query.mockResolvedValue({ rows: [{
      ...
      error_code: null, error_message: null, attempts: 1, run_revision: 1, queue_confirmed_at: new Date(),
    ...
    expect(await response.json()).toMatchObject({ analysis: { result: { inventory: [] } } });
  });
