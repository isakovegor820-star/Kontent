...
Проверено 9 сентября 2026 года. Изменения подготовлены на базе релизного кандидата `3eabb1d`; этот отчёт не подтверждает развёртывание на рабочем сайте.
...
- Полный набор модульных тестов: 624 файла, 3451 тест на момент общего прогона. После последующих исправлений отдельно повторены затронутые проверки поиска и интерфейса.
- PostgreSQL: 9 интеграционных сценариев — совпадение ленты, суммы и графика; неизвестные счётчики и настоящие нули; коэффициент ×4; дедупликация; изоляция запросов и владельцев; пагинация; точные интервалы.
...
- TypeScript, ESLint, проверка 116 миграций и запрета пропущенных тестов прошли. Производственная сборка Next.js прошла.
...
Для браузерного прогона нужны локальные PostgreSQL с pgvector, Redis и Chrome. Перед выпуском требуется применить миграцию `20261013_trends_search_scope.sql` штатным процессом миграций.
