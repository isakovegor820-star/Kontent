...
export const BOT_HELP_TEXT =
  ...
  "• Подключение — проверить связь и добавить Telegram-канал прямо из бота\n" +
...
export function formatBotChannelConnectPrompt(input) {
  const projectName = String(input?.projectName || "Текущий проект");
  return [
    "✦ Подключить Telegram-канал",
    `Проект: ${projectName}`,
    "",
    "Нажми кнопку и выбери канал, которым управляешь. Telegram сам добавит меня с единственным нужным правом — публиковать сообщения.",
    "",
    "Возвращаться на сайт и вводить @адрес не нужно. После выбора канала укажи проект в сообщении бота. Я проверю доступ и подтвержу подключение здесь.",
  ].join("\n");
}
