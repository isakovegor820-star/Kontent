export async function claimPublicationLease(pool, input) {
  const scheduled = await pool.query(
    ...
    [input.postId, input.leaseToken, input.scheduleRevision, input.overdueCutoff],
  );
  ...
  const retry = await pool.query(
    `update posts p
      ...
      returning p.id, p.user_id, p.channel_id, p.text, p.media, p.attempts,
                p.publication_operation_id`,
    [input.postId, input.leaseToken, input.scheduleRevision],
  );
...
export async function beginProviderCall(pool, input) {
  const started = await pool.query(
    `update posts
      ...
      returning id`,
    [input.postId, input.scheduleRevision, input.leaseToken],
  );
