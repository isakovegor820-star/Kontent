...
export async function refineSiteProfile(pool, { profileId, force = false }, dependencies = {}) {
  ...
  if (attempts > 3) {
    await ownedTaskTransaction(pool, identity, (client) => client.query(
  ...
  try {
    ...
    if (inventory.length) {
      ...
      try {
        ...
        }, { allowFallback: true, timeoutMs: 90_000, signal: heartbeat.signal });
    ...
    return ownedTaskTransaction(pool, identity, async (client) => {
      await client.query(
...
export async function interpretSiteReport(pool, { reportId, force = false, revision = null }, dependencies = {}) {
  ...
  const report = (await pool.query(
    `select r.id, r.site_id, r.kind, r.payload, r.interpretation, r.interpretation_status, r.interpretation_revision,
            s.user_id, s.brand_name, s.confirmed_domain, s.status as site_status, p.topics
       from site_reports r join sites s on s.id = r.site_id left join site_profiles p on p.id = r.profile_id
      where r.id = $1 and r.status = 'ready'`, [reportId],
  )).rows[0];
