...
Статус: спецификация согласована на уровне продуктовых решений, к реализации не приступали.
...
1. Миграция: `sites`, `site_profiles`, расширение `knowledge_sources/chunks` под `site_id`.
2. `coverage_mode = 'profile'` в `site-analysis`: классификация страниц, темы, пробелы, `linkable_pages`, `technical`.
3. Индексация страниц сайта в базу знаний (`site_page`).
4. API `/api/sites` (создать, подтвердить домен, профиль), UI карточки сайта.
5. Отчёт `initial_audit` и его экспорт — первый видимый результат для пользователя.
...
1. TLS и домены для хостируемого раздела: автоматизация выпуска сертификатов или ручной процесс на старте. Решить до начала этапа 2.
2. Тарификация: зонд GEO расходует бюджет движков; нужно решить, входит ли он в текущие лимиты `ai_usage` или выделяется отдельно.
3. Интеграции Яндекс.Вебмастер / GSC — отдельная спецификация после этапа 5; без них раздел `seo` отчёта остаётся on-page.
4. Изображения для статей: переиспользовать `media-generation` как есть или добавить обложки с брендингом из `project_brand_kits` — уточнить после первых материалов.
