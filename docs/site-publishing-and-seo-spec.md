...
Статус: этапы 1–5 реализованы. Схема — миграции `20261006_sites_and_site_profiles.sql` и `20261007_site_publishing.sql`; библиотеки — `src/lib/site-profile/`, `src/lib/site-report/`, `src/lib/site-destinations/`, `src/lib/site-articles/`, `src/lib/site-probe/`, `src/lib/site-hosted/`, `src/lib/sites/`; worker — `worker/site-profile-persistence.mjs`, `worker/site-articles-worker.mjs`, `worker/site-visibility-probe.mjs`, `worker/site-scheduler.mjs`, `worker/embeddings.mjs`; API — `src/app/api/sites/**`; UI — `src/app/app/sites/`, публичный раздел — `src/app/hosted/`. Отклонения реализации от исходного проекта зафиксированы в разделе 14.
...
## 14. Отклонения реализации от исходного проекта

Зафиксированы при реализации этапов 2–5 (2 сентября 2026 года). Каждое — техническое решение в рамках делегированных полномочий; продуктовые решения раздела 3 не менялись.

1. **Реестр назначений отделён от `provider-capabilities.mjs`.** Каталог `PROVIDER_IDS` описывает каналы соцсетей и перечисляется в UI и проверках публикаций постов; добавление `wordpress`/`site_hosted` в него меняло бы поведение композитора. Возможности назначений живут в `src/lib/site-destinations/contract.mjs` (`SITE_DESTINATION_CAPABILITIES`, `payloadKinds: ["article"]`) и реализуют тот же контракт исходов (`success / definite_failure / delivery_unknown / rate_limited / auth_failed`) из `social-provider-contract.mjs`.
2. **Операции публикации статей — отдельная таблица `site_article_publications`**, а не `publication_operations`. Существующая таблица привязана к постам (`text`, `media`, `destination_ids`, календарь); у статьи другой payload (slug, HTML, JSON-LD) и другое назначение. Идемпотентность, reconcile по slug, лимит попыток и fail-closed исходы воспроизведены один в один; очередь — `site-articles`, а не `publish`.
3. **Проверка качества материала реализована в `site-articles/generation.mjs` (`validateArticle`)**, а не как профиль `article` в `post-quality.mjs`: тот модуль оценивает пост для соцсети и его правила (длина под Telegram, хэштеги) статье не подходят. Требования типов (прямой ответ ≤ 60 слов, H2, ссылка на первоисточник, блок «что это значит», запрет непроверяемых заявлений) — детерминированные проверки с одной повторной генерацией по списку ошибок.
4. **Проверка дублей двухконтурная.** Векторный контур (pgvector, пороги 0.86 / 0.78) работает, когда эмбеддинги доступны; лексический (шинглы слов + косинус tf, пороги 0.55 / 0.32, откалиброваны на фикстурах) работает всегда. Решение принимается по худшему контуру, поэтому недоступность модели эмбеддингов не отключает защиту.
5. **Индексация в базу знаний — через существующий крон `knowledge-index`.** Страницы сайта (`site_page`), опубликованные материалы (`site_publication`) и Markdown отчётов (`site_report`) создаются как `knowledge_sources` со статусом `pending`; `reconcilePendingKnowledgeSources` подхватывает их каждые 5 минут, `indexSource` пишет `site_id` в чанки. Отдельной очереди для сайтов не заводилось.
6. **Хостируемый раздел отдаётся через `proxy.ts`.** Host `<slug>.<AURORA_SITES_DOMAIN>` переписывается на `/hosted/<slug>/…`; на таком хосте `/app`, `/api` и прямые `/hosted` — 404. Раздел отдаётся только для активного, подтверждённого сайта с активным назначением `site_hosted`. Логи отдачи страниц в `product_events` не пишутся: это отложено до появления потребителя (аналитика хостируемого раздела).
7. **Планирование материалов — ежедневное, квоты — недельные.** Крон `site-daily` (05:20 МСК) планирует по каждому сайту с учётом уже созданных за неделю материалов и общего пула «разбор или гид»; недельный запуск давал бы всплеск материалов в один день. Источники: пробелы профиля, `audience_questions` проекта, `rss_items` лент пользователя за 7 дней, опубликованные посты каналов проекта за 14 дней.
8. **Зонд видимости — в очереди `site-articles`** (job `probe`), а не в отдельной очереди; ежемесячные отчёты — крон `site-monthly` (07:00 МСК 1-го числа), отчёт по запросу — job `report` за последние 30 дней с типом `on_demand`. Пересборка профиля — раз в 14 дней или после 5 публикаций (`site-daily`).
9. **Статусы рекомендаций считаются по ключам.** Рекомендация прошлого отчёта помечается `done`, если её ключ (`technical:<check>` или `gap:<key>`) отсутствует в текущем наборе; открытые переносятся с `sinceReportId`. Это детерминированно и не требует ручной отметки.
10. **Учётные данные WordPress проверяются до сохранения.** `PUT /api/sites/:id/destinations` делает живой `GET /wp/v2/users/me?context=edit` и требует `publish_posts`; неверные данные в базу не попадают даже в зашифрованном виде. Хранение — `encryptToken` из `token-crypto.mjs` с provider `site_destination`.

## 15. Слой модели поверх детерминированного ядра

Добавлен 3 сентября 2026 года по согласованию: «факты — скрипт, смысл — модель». Детерминированные профиль и отчёт остаются источником истины; модель добавляет два кэшируемых слоя.

1. **Классификатор страниц и тем** (`src/lib/site-ai/interpretation.mjs`, job `refine` в очереди `site-articles`, surface `site-page-classifier`, небиллинговый). После записи профиля worker анализа ставит задание; модель видит только URL, заголовок и первые подзаголовки, возвращает уточнённые типы страниц (только из списка `SITE_PROFILE_PAGE_TYPES`) и кластеры синонимичных тем (только из известных ключей). Результат хранится в `site_profiles.ai_classification`, профиль пересобирается тем же `buildSiteProfile` с параметром `classification`, стартовый аудит по этому профилю пересобирается по той же формуле. Сбой классификатора оставляет детерминированный профиль без изменений.
2. **Интерпретация отчёта** (job `interpret`, surface `site-report-interpretation`, под резервацией лимита пользователя, kind `site_report_interpretation`). Промпт получает только факты payload и ключи открытых рекомендаций; ответ — `summary`, `whatItMeans`, `startWith` (приоритизация по ключам рекомендаций), `watchOut`. Валидатор удаляет формулировки-обещания (`FORBIDDEN_CLAIMS`), ссылки на несуществующие рекомендации и дубли; пустая после чистки интерпретация помечается `failed`. Хранится в `site_reports.interpretation` со статусом; выводится отдельным блоком «Интерпретация Авроры» в профиле, в карточке отчёта и в экспорте, всегда с дисклеймером о том, что расчёт — детерминированный.
3. **Что не отдано модели:** on-page проверки и оценки, пороги дублей, шаблоны сводки и ограничений, валидатор материалов. Ежемесячные отчёты интерпретируются тем же job после сборки; догоняющий проход `enqueuePendingInterpretations` входит в `site-daily`.
