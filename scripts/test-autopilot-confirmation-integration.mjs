...
async function insertPlan(pool, userId, channelId, items) {
...
async function insertOperation(pool, userId, channelId, planId, key, revision, hash) {
  return Number((await pool.query(
    `insert into autopilot_approval_operations
     ...
     values ($1, $2, $3, $4, $5, $6, 'web', 'processing', '{}'::jsonb)
     returning id`,
    [userId, channelId, planId, revision, hash, key],
  )).rows[0].id);
}

async function sideEffectCounts(pool, userId, queue) {
...
try {
  ...
  const channels = (await pool.query(
    `insert into channels (user_id, network, title, handle, is_active)
     values ($1, 'tg', 'QA channel A', 'qa_gate2_a', true),
            ($1, 'tg', 'QA channel B', 'qa_gate2_b', true)
     returning id`,
    [userId],
  ...
  for (const mutation of ["draft", "date", "quality"]) {
    ...
    const planId = await insertPlan(pool, userId, channelA, original);
    ...
    await pool.query(
      `insert into autopilot_approval_previews
       ...
       values ($1, $2, $3, $4, 1, $5, $6::jsonb, now() + interval '5 minutes')`,
      [hashAutopilotPreviewToken(token), userId, channelA, planId, preview.hash, JSON.stringify(preview)],
    );
  ...
  const crossChannelPlan = await insertPlan(pool, userId, channelA, makeItems("channel isolation"));
  const crossToken = createAutopilotPreviewToken();
  await pool.query(
    `insert into autopilot_approval_previews
       (token_hash, user_id, channel_id, plan_id, plan_revision, preview_hash, snapshot, expires_at)
     values ($1, $2, $3, $4, 1, $5, '{}'::jsonb, now() + interval '5 minutes')`,
    [hashAutopilotPreviewToken(crossToken), userId, channelA, crossChannelPlan, "0".repeat(64)],
  );
  const crossChannelMatch = await pool.query(
    `select token_hash from autopilot_approval_previews
      where token_hash = $1 and user_id = $2 and channel_id = $3 and plan_id = $4`,
    [hashAutopilotPreviewToken(crossToken), userId, channelB, crossChannelPlan],
  );
  assert.equal(crossChannelMatch.rowCount, 0, "channel B accepted channel A preview token");

  const raceItems = makeItems("parallel");
  const racePlan = await insertPlan(pool, userId, channelA, raceItems);
  ...
  const operationIds = await Promise.all([
    insertOperation(pool, userId, channelA, racePlan, "qa-race-a", 1, raceHash),
    insertOperation(pool, userId, channelA, racePlan, "qa-race-b", 1, raceHash),
  ]);
  ...
  const duplicateKeyResults = await Promise.all(["a", "b"].map(() => pool.query(
    `insert into autopilot_approval_operations
       (user_id, channel_id, plan_id, plan_revision, preview_hash,
        idempotency_key, actor_type, status, request_snapshot)
     values ($1, $2, $3, 1, $4, 'qa-same-idempotency-key', 'web', 'processing', '{}'::jsonb)
     on conflict (user_id, idempotency_key) do nothing returning id`,
    [userId, channelA, crossChannelPlan, "0".repeat(64)],
  )));
  assert.equal(duplicateKeyResults.reduce((sum, result) => sum + Number(result.rowCount), 0), 1);

  const effects = await sideEffectCounts(pool, userId, publishQueue);
