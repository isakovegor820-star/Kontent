...
async function selectProject(page, projectId, waitFor) {
  ...
  await waitFor(async () => !await selector.isDisabled(), "project selector remained disabled", TIMEOUT);
  if (Number(await selector.inputValue()) !== projectId) await selector.selectOption(String(projectId));
  await waitFor(async () => await tabProject(page) === projectId && Number(await selector.inputValue()) === projectId,
    `tab did not adopt project ${projectId}`, TIMEOUT);
}
...
export async function runProjectCalendarCoverage({ page, context, pool, userId, sharedProjectId, legacyProjectId, sharedChannelId, waitFor, artifactDir, closeCompletedPage = (page) => page.close() }) {
  ...
  try {
    ...
    await selectProject(page, sharedProjectId, waitFor);
    await other.goto("/app/calendar");
    await selectProject(other, legacyProjectId, waitFor);
    ...
    await page.goto(`/app/calendar#calendar-real-${seeded.postId}`);
    ...
    await page.goto("/app/calendar");
    await selectProject(page, legacyProjectId, waitFor);
    await selectProject(page, sharedProjectId, waitFor);
