...
function fixture({invalidProjectHeader=false, continuationError=null}={}) {
  ...
  const button=(name)=>({
    ...
    click:async()=>{
      if(name==='Улучшить') {
        ...
        const request={url:()=>baseUrl+'/api/ai/generate',method:()=>'POST',headers:()=>({'idempotency-key':key,'x-aurora-project-id':invalidProjectHeader?'999':'2'}),
        ...
        const pending=gate({request:()=>ackRequest,continue:async()=>{
          ...
          acknowledged=true;ackResolve(ack);
        }});
