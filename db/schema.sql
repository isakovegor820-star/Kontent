...
-- Weekly growth desk: diagnosis is computed, moves stay stable for the Moscow week.
...
-- ------------------------------------------------ Account settings and safe previews

-- Account-facing preferences stay separate from the per-channel content brief so
-- profile edits cannot rewrite the prompt configuration of a publishing channel.
...
-- Preview runs have a dedicated quota and never reserve the normal ai_usage allowance.
-- Define them only after the channels tenant key exists for the composite foreign key.
...
begin
  if new.is_active = true and new.status = 'active' then
    insert into channel_feature_flags (
      project_id, channel_id, feature_key, enabled, enabled_at
    ) values (
      new.project_id, new.id, 'content_intelligence_release_1', true, now()
...
-- Content-free access trail for global administrative observations. Project audit_events
-- remains the source for tenant mutations; this table records read-only operations-center access.
...
-- ------------------------------------------------ Autopilot project settings identity
...
-- Preserve the legacy conflict arbiter so the previous application release remains
-- runnable after a forward migration and can be used for an application-only rollback.
...
-- ------------------------------------------------ Monthly campaign full regeneration
...
-- 20261009_admin_account_controls: administrator account controls.
...
-- ------------------------------------------------ ИИ-интерпретация профиля и отчётов сайта
-- Зеркало db/migrations/20261010_site_ai_interpretation.sql.
...
-- ------------------------------------------------ Autopilot schedule continuation
-- schedule_mode = 'continue' keeps already scheduled posts and places the new plan after
-- their coverage; 'replace' is the explicit user choice to cancel them. coverage_until is
-- the end of the planned window captured when the build started (slot-shift source).
